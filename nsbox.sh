...
Аргументы, обязательные для длинных параметров, обязательны и для коротких.
  -c, --command 'программа'     Запу
  стить программу в изолированном контейнере
  -l, --lower 'путь'            Указать каталог для нижнего слоя ФС контейнера, по умолчанию корень ФС хоста
...
if [ -z "$CONTAINER_NAME" ]; then
    ...
    echo -n "Существующие контейнеры: "
    find "$REGISTRY_DIR" -type f -name *.img -print0 | xargs -0 -n 1 basename -s .img | tr '\n' ' '
    echo -e "\n"
    usage
fi
